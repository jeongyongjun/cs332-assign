...
trait Solver extends GameDef {
  ...
  def newNeighborsOnly(neighbors: Stream[(Block, List[Move])],
    ...
    neighbors.collect {
      case neighbor @ (block, _) if !explored.contains(block) => neighbor
    }
  }
  /**
  ...
  def from(initial: Stream[(Block, List[Move])],
    ...
    if (initial.isEmpty) Stream.empty
      ...
      val more = initial.flatMap { case (block, history) =>
        neighborsWithHistory(block, history).filter { case (b, _) => !explored.contains(b) }
      }
  ...
  lazy val pathsFromStart: Stream[(Block, List[Move])] = {
    val init = Stream((startBlock, List[Move]()))
