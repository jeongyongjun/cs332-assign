...
trait NodeScala {
   ...
   */
  private def respond(exchange: Exchange, cancelToken: CancellationToken, content: Response): Unit = {
    val contentIterator = content.buffered
    while (contentIterator.hasNext && !cancelToken.isCancelled) {
      exchange.write(contentIterator.next())
    }
    exchange.close()
  }
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    val routeListener = createListener(relativePath)
    val listenerSub = routeListener.start()

    val serverSub = Future.run() { cancelToken =>
      async {
        while (!cancelToken.isCancelled) {
          val (request, exchange) = await(routeListener.nextRequest())
